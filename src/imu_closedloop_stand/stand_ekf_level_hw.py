...
HOW TO RUN
    cd ~/Documents/can_motor_control/the August track

    python3 stand_ekf_level_hw.py --self-test        # offline gates, no hardware

    # 1. open-loop A/B -- reproduces stage 2 at this stand height
    sudo chrt -f 50 python3 stand_ekf_level_hw.py --level-gain 0
    # 2. leveling live at half authority
    sudo chrt -f 50 python3 stand_ekf_level_hw.py --level-clamp 0.006
    # 3. full run
    sudo chrt -f 50 python3 stand_ekf_level_hw.py
...
#
#     python3 selftest/test_stand_ekf_level.py     # the same thing, run directly
