...
`vmc/ekf_stand_hw.py` and `position_stand_crawl/stand3_hold_hw.py`.  The estimator runs
...
Run:
    python3 stand_ekf_verify_hw.py --self-test            # no hardware needed
    sudo chrt -f 50 python3 stand_ekf_verify_hw.py \
...
Then replay for the gate-C8 report:
    python3 ../state_estimator/hw_replay.py verify.npz --static
"""
...
def _find_up(name, start=None):
    ...
    into imu_closedloop_stand/, which silently broke every one of these imports
...
def selftest_dir():
    """Locate selftest/, wherever the runners currently sit relative to it."""
    root = _find_up("selftest")
    return os.path.join(root or os.path.dirname(_HERE), "selftest")
...
#
#     python3 selftest/test_stand_ekf_verify.py     # the same thing, run directly


def self_test(stand_height):
    """Delegate to the suite in selftest/.  Lazy: a hardware run never loads it."""
