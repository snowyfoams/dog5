...
THE JOINT-SPACE FLOOR IS NOT OPTIONAL
    ...
    with a fixed point, exactly as torque_stand does, and q_ref is the pose
...
RUN
    python3 dog5_trot_quasi_static_model/controller.py --self-test
"""
...
from dog5_trot_quasi_static_model import config as cfg
from dog5_trot_quasi_static_model import leg_kin
from dog5_trot_quasi_static_model import gait as gait_mod
from dog5_trot_quasi_static_model import balance_qp
from dog5_trot_quasi_static_model import swing as swing_mod
...
def stance_torque(leg: int, q, f_world, R_wb, J=None) -> np.ndarray:
    ...
    the floor; torque_stand/force_totorque.py has carried the minus since it
...
def leg_gravity_torque(leg: int, q, R_wb) -> np.ndarray:
    ...
    TAU_MAX, against a -J^T f that is itself only a few Nm.  torque_stand
...
class TrotController:
    ...
    def _update_q_ref(self, state, contact, feet_body, Js):
        ...
        neither direction on 2026-08-18, which is exactly what torque_stand's
...
def self_test():
    ...
    check("...and stance_torque agrees with torque_stand's verified -J^T f",
