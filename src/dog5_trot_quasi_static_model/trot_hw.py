...
THIS FILE IS torque_stand/stand_torque_mode.py WITH ONE STAGE ADDED.
...
RUN -- supported robot, hand on SPACE
    ...
    sudo HOME=$HOME chrt -f 50 python3 dog5_trot_quasi_static_model/trot_hw.py
    ...
    WALKING (new 2026-08-25) is the same placement channel driven open loop,
    and it is FLOWN BY walk_demo.py, NOT BY THIS RUNNER: run()'s walk path
    is gated on args.walk_step, which main() pins to None, so trot_hw and
    trot_demo trot in place whatever config.py's WALK block says.  When
    walk_demo sets it, WALK_STEP_M is added to every swing's landing point
    -- +x from T until the R key, which walks the SAME NUMBER of swings
    back (a per-leg ledger, WalkSequence -- counted in swings, not seconds,
    because the settles make wall time and distance different clocks), then
    the runner latches the same orderly exit T does and returns to HOLD by
    itself.  The body follows the feet: q_ref pins every stance foot at its
    crouch x/y, so a foot that lands ahead is pulled back toward nominal and
    the TRUNK is what moves -- there is no new force law anywhere in it.
    The step rides through raibert_step_body's clamps beside the Raibert
    terms, so it SHARES the reach room the paragraph above describes: the
    banner warns when WALK_STEP_M alone exceeds it.  See config.py's WALK
    block for the speed arithmetic and why v_cmd stays out of the kv term.

Keys: ENTER = rise / re-engage from limp, T = trot from HOLD / back to HOLD
      from TROT, R = walk the outbound swings back (walk_demo only),
      ...
      diagonal.  The ORDERLY way out of a trot is T again: the exit is
      LATCHED and taken at the next sweep with all four feet scheduled down,
      so no leg is abandoned mid-swing; then HOLD, then P parks as it always
      did.  X still cuts torque at height at any instant.  RUN THE TROT WITH
      --park-on-stop if you want X to put it down in position mode instead
      of handing you the weight.
"""
...
_AUG = os.path.dirname(_HERE)                               # src/
_WEEK2 = os.path.join(_AUG, "torque_stand")
_ROOT = dog5_paths.SRC
_REPO = dog5_paths.SRC
...
def _add_fdilink_root():
    """Locate the fdilink_imu vendor SDK without trusting $HOME.

    Delegates to dog5_paths, which is the single implementation; this wrapper
    is kept so the call sites below read unchanged.
    """
    return dog5_paths.add_fdilink_root()
...
def raibert_step_body(i, v_world, C, t_stance, kv, z_des, foot_xy,
                      v_cmd=(0.0, 0.0), step_ff=(0.0, 0.0)):
    """Trunk-frame x/y offset from a foot's resting site to where it should
    LAND, clamped to the leg's reach.  (2,)

    step_ff IS THE WALK: a feedforward landing offset added BEFORE the
    clamps, so the walk and the drift correction spend the same reach room
    and neither can smuggle a foot past the inward cap.  (0, 0), the
    default, is bit-for-bit the pre-walk placement.
    ...
    step = (np.asarray(step_ff, dtype=float).reshape(2)
            + v_b[:2] * (0.5 * float(t_stance))
...
class WalkSequence:
    """The R-key walk: +step_m per swing from T, and after reverse() every
    leg gives back EXACTLY the swings it took, -step_m each.  Then `done`.

    PURE COUNTING, NO CLOCK.  The old sequence was two wall-clock timers
    (fwd_s, then rev_s), and wall-equal is not distance-equal: trot_demo's
    settles land on a cycle-index schedule, so the same seconds do not hold
    the same number of swings on the way out and the way back.  Counting
    latched swings PER LEG is the quantity that actually moved the robot --
    each latch displaces that foot step_m -- so equal counts return every
    foot to where it started, whichever diagonal was mid-cycle when R fell.

    step_for(i) is called ONCE per liftoff edge of leg i, at the same latch
    that takes the Raibert step, and returns the x feedforward for THAT
    swing: +step_m while walking out (and one more owed), -step_m while an
    owed swing remains on leg i, 0 after.  `done` is the caller's cue to
    latch the SAME orderly exit T does, so the sequence parks itself through
    the existing four-foot-window machinery rather than growing a second
    exit path.  A plain object with no clock so the ledger is testable
    without a robot.
    """

    def __init__(self, step_m):
        self.step_m = float(step_m)
        self.owed = [0, 0, 0, 0]         # +x swings each leg still owes back
        self.reversing = False

    def step_for(self, i):
        """The x feedforward for the swing leg i is latching right now."""
    ...
    def reverse(self):
        """The R key: stop walking out, start giving the swings back."""
        self.reversing = True

    @property
    def done(self):
        """True once reversing and every owed swing has been latched back.
        (The last one may still be mid-air -- the exit machinery this cues
        already waits for the next four-foot window.)"""
        return self.reversing and sum(self.owed) == 0
...
def run(args):
    ...
    if args.walk_step is not None:
        print(f"  walk: T walks +x at {args.walk_step*1e3:.0f} mm/swing "
              f"until R; R walks the SAME number of swings back, "
              f"then exits to HOLD by itself.")
        if args.walk_step > _room:
            print(f"  WARNING: WALK_STEP_M {args.walk_step*1e3:.0f} mm exceeds "
                  f"the {_room*1e3:.1f} mm of reach room at this height -- "
                  f"every step clamps short.  Crouch (STAND_TRUNK_BOTTOM_M) "
                  f"or shrink the step.")
    ...
    try:
        ...
        with motorbus.MotorBus(MOTOR_IDS, dirs=base.MOTOR_DIRECTIONS) as mb:
            ...
            limp = False
            armed = False
            # T DURING TROT LATCHES AN ORDERLY EXIT.  The switch itself waits
            # for a sweep where the schedule has all four feet down, so no
            # leg is abandoned mid-swing -- see the block after the keys.
            trot_exit = False
            # THE WALK LEDGER, one per T (None = walk off or not trotting).
            # Made fresh each time TROT starts so a second T cannot inherit
            # swings owed from the last one.
            walk = None
            load_sum = float("nan")
            ...
            while stop is None:
                ...
                if j == 0:
                    ...
                    if pressed in ("t", "T") and not limp:
                        if stage == "HOLD":
                            stage, t0 = "TROT", now
                            trot_exit = False
                            gait.reset(now)
                            # gait.reset restarts the clock, so every leg is
                            # about to produce a liftoff edge.  Clearing here
                            # rather than relying on the stance branch means a
                            # second T after a limp cannot re-apply a step
                            # planned before it.
                            step_xy[:] = 0.0
                            was_swing[:] = False
                            print(f"[trot] TROT: {gait}.  Feet are leaving "
                                  f"the ground.  SPACE limps, X stops.  T "
                                  f"again returns to HOLD; P parks from "
                                  f"there.")
                            if args.walk_step is not None:
                                walk = WalkSequence(args.walk_step)
                                print(f"[walk] walking +x at "
                                      f"{args.walk_step*1e3:.0f} mm/swing.  "
                                      f"R walks the same swings back, then "
                                      f"HOLD by itself.")
                        elif stage == "TROT" and not trot_exit:
                            # THE EXIT IS LATCHED, NOT TAKEN: switching stages
                            # with a leg mid-swing would hand that leg the
                            # stance law in mid-air.  The block below takes it
                            # at the first all-four-down sweep.
                            trot_exit = True
                            print("[trot] exit latched: HOLD at the next "
                                  "four-foot window, then P parks.")

                    if pressed in ("r", "R") and stage == "TROT" \
                            and not limp and walk is not None \
                            and not walk.reversing:
                        # THE RETURN KEY.  The ledger flips to paying back;
                        # nothing else changes -- same clock, same latch,
                        # same clamps, only the sign of the feedforward.
                        walk.reverse()
                        print(f"[walk] R: walking back -- "
                              f"{sum(walk.owed)} swings to give back "
                              f"({'/'.join(str(c) for c in walk.owed)} "
                              f"per leg), then HOLD by itself.")

                    # the walk parks itself: once every owed swing is latched
                    # back, cue the SAME orderly exit the T key takes.
                    if stage == "TROT" and walk is not None and walk.done \
                            and not trot_exit:
                        trot_exit = True
                        print("[walk] sequence done -- out and back, even.  "
                              "HOLD at the next four-foot window; P parks.")

                    if trot_exit and stage == "TROT" \
                            and gait.contact(now).all():
                        stage, t0 = "HOLD", now
                        trot_exit = False
                        # the same latch hygiene as entering TROT: a stale
                        # step must not survive into the next T, and neither
                        # may a half-paid walk ledger.
                        step_xy[:] = 0.0
                        was_swing[:] = False
                        walk = None
                        print("[trot] back in HOLD -- all four feet down.  "
                              "P parks, T trots again.")
                                ...
                                if stage == "TROT":
                                    ...
                                    for i in range(4):
                                        ...
                                        if (args.raibert is not None
                                            ...
                                            # With placement OFF the Raibert
                                            # terms are zeroed and the walk
                                            # step rides the clamps alone.
                                            # walk.step_for(i) is the walk's
                                            # ONLY moving part -- it debits
                                            # the ledger, so it is called
                                            # exactly once per edge, HERE.
    ...
    if log:
        ...
        if args.log:
            np.savez_compressed(
                ...
                # the walk this run flew (nan = off).  step_xy above already
                # carries what each swing was actually asked, walk step and
                # Raibert correction summed and clamped together; the walk's
                # share is +-this per swing, and the R press is the sweep
                # where step_xy's walk sign flips -- the timers the old
                # walk_fwd_s/walk_rev_s keys recorded no longer exist.
                walk_step_m=(float("nan") if args.walk_step is None
                             else float(args.walk_step)),
...
def self_test():
    ...
    # -- walking: the feedforward step, and the sequence that drives it ----
    # The walk is the SAME latch and the SAME clamps, so in-place behaviour,
    # the inward cap and the reach clamp need no re-proof -- what is proved
    # here is the new term and the schedule that feeds it.
    w0 = raibert_step_body(0, np.zeros(3), np.eye(3), T_st, 0.0, zh, fxy,
                           step_ff=(0.006, 0.0))
    check("a walk step passes through whole when there is room",
          abs(w0[0] - 0.006) < 1e-12 and abs(w0[1]) < 1e-12,
          f"asked 6.0 mm forward, planned {w0[0]*1e3:.1f} mm")
    w_big = raibert_step_body(0, np.zeros(3), np.eye(3), T_st, 0.0, zh, fxy,
                              step_ff=(0.5, 0.0))
    land_w = np.array([fxy[0][0] + w_big[0], fxy[0][1] + w_big[1], z_site])
    check("an oversized walk step meets the same reach clamp as a wild drift",
          float(np.linalg.norm(land_w - hip0)) <= 0.95 * cfg.LEG_REACH + 1e-9
          and w_big[0] < 0.5,
          f"asked 500 mm, planned {w_big[0]*1e3:.1f} mm")
    check("step_ff defaults to zero: the placement path is bit-for-bit "
          "unchanged with the walk off",
          np.array_equal(
              raibert_step_body(0, np.array([0.02, 0.0, 0.0]), np.eye(3),
                                T_st, cfg.RAIBERT_KV, zh, fxy),
              raibert_step_body(0, np.array([0.02, 0.0, 0.0]), np.eye(3),
                                T_st, cfg.RAIBERT_KV, zh, fxy,
                                step_ff=(0.0, 0.0))))
    ws = WalkSequence(0.008)
    fwd = [ws.step_for(i) for _ in range(3) for i in range(4)]
    check("walking out: every latch is +step, and none of it is done",
          all(s == 0.008 for s in fwd) and ws.owed == [3, 3, 3, 3]
          and not ws.done and not ws.reversing,
          "3 cycles out: each leg owes 3 swings back")
    ws.step_for(0)                    # an uneven cycle: R falls mid-cycle
    ws.reverse()
    check("R flips the ledger: -step per latch until each leg is even",
          ws.step_for(0) == -0.008 and ws.owed[0] == 3
          and all(ws.step_for(1) == -0.008 for _ in range(3))
          and ws.step_for(1) == 0.0 and not ws.done,
          "leg 1 paid its 3 back and steps 0 after; leg 0 still owes 3")
    for i in (0, 0, 0, 2, 2, 2, 3, 3, 3):
        ws.step_for(i)
    check("done exactly when EVERY leg has given back its own count -- "
          "swings, not seconds, R mid-cycle included",
          ws.done and ws.owed == [0, 0, 0, 0]
          and ws.step_for(2) == 0.0,
          "`done` latches the same orderly exit the T key does")
    ws0 = WalkSequence(0.008)
    ws0.reverse()
    check("R before any forward swing owes nothing and is done at once",
          ws0.done and ws0.step_for(1) == 0.0)
    check("the configured walk step fits the reach room at the height flown",
          not cfg.WALK_ON or cfg.WALK_STEP_M < _step_room(zh),
          f"WALK_STEP_M {cfg.WALK_STEP_M*1e3:.0f} mm vs "
          f"{_step_room(zh)*1e3:.1f} mm of room at "
          f"{zh*1e3:.0f} mm trunk-bottom -- the banner warns, this refuses")
...
def main():
    ...
    # WALKING IS walk_demo's, AND THIS PIN IS WHAT KEEPS IT THAT WAY.  run()'s
    # whole walk path is gated on args.walk_step, so None here means neither
    # THIS runner nor trot_demo -- which defers to this main() -- can walk,
    # whatever config.py's WALK block says.  walk_demo.py wraps run() and is
    ...
    if not 0.0 < args.tau_max <= cfg.TAU_HARD_NM:
        ap.error(f"--tau-max must be in (0, {cfg.TAU_HARD_NM}]")
