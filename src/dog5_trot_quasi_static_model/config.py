...
# the frame torque_stand controls.  p_com is the CoM because that is the point
...
# stance and gait
# ===========================================================================
# Trunk-origin height above the foot plane.  torque_stand measures floor to
...
GAIT_PERIOD = 1.2             # s, one full cycle
...
# ===========================================================================
# trot_demo: the settle-trot (demo workaround while the roll stays open)
# ===========================================================================
# These numbers do NOT solve the roll -- they buy a stable-looking demo by
# giving the stand-side controller time to undo what the cycles do to it.
# trot_demo.py replays trot_hw's schedule UNCHANGED and, once every
# DEMO_SETTLE_EVERY full gait cycles -- right after that cycle's second
# touchdown has ramped to full support -- FREEZES the gait clock: all four
# contact weights sit at 1 for DEMO_SETTLE_S, the QP re-distributes force
# over four feet and the attitude spring pulls the roll back to the
# setpoint, then the next cycle is released.  The settle is FIXED and
# PERIODIC (it used to grow every cycle; that accumulation is gone since
# 2026-08-25): trot DEMO_SETTLE_EVERY cycles, re-level for DEMO_SETTLE_S,
# repeat.
DEMO_GAIT_PERIOD = GAIT_PERIOD    # s, SAME cycle as trot_hw -- the demo is a
                                  # replica, only the settle is added
DEMO_SETTLE_S = 0.2               # s, the four-foot attitude settle;
                                  # 0 = exactly trot_hw
DEMO_SETTLE_EVERY = 2             # settle once every THIS many full cycles
                                  # (1 = after every cycle, the old cadence)
# THE LEAD ALTERNATES, flipped INSIDE each settle.  The robot walks out of
# place, and any drift that depends on which diagonal swings first keeps its
# sign every cycle -- so it integrates.  Flipping the lead each cycle makes
# that component alternate sign and cancel over pairs of cycles instead.
# The flip happens mid-settle, where BOTH diagonals are mid-stance at full
# weight (phases 0.15/0.65 just swap sides), so no contact state and no
# weight changes at the flip instant.  Drift that is the SAME whichever
# diagonal leads (e.g. every foot landing displaced the same way) is NOT
# cancelled by this -- if the robot still walks with this on, the drift is
# order-independent and the fix is elsewhere.
DEMO_ALTERNATE_LEAD = True        # False = fixed lead, the previous demo
# RAIBERT PLACEMENT, DEMO'S OWN SWITCH.  The runner's whole placement path
# (liftoff-edge latch, reach clamp, inward cap) already exists behind
# args.raibert; trot_hw gates it on RAIBERT_ON above, the demo gates it on
# THIS, so the two runners stay independent A/Bs.  Placing the foot
# downstream of the measured drift is the mechanism that answers "the trot
# walks out of place" -- the alternating lead cancels order-dependent drift,
# this one answers the order-INDEPENDENT remainder.  Gain is the shared
# RAIBERT_KV below; the npz records raibert_kv (nan = off).
DEMO_RAIBERT_ON = True            # False = feet land where they lifted
...
# torque_stand uses 60 Nm/s, which was sized so a stand could answer a push.
...
# ===========================================================================
# walking: a feedforward step -- out on T, back on R, then stop
# ===========================================================================
# THE WALK IS THE PLACEMENT CHANNEL DRIVEN OPEN LOOP.  The landing point is
# the ONLY horizontal channel this stack has (kd_xy = 0 above, and the QP is
# asked for zero net Fx/Fy), and the body follows the feet: q_ref_for_height
# pins every STANCE foot's x/y at its crouch value, so a foot that lands
# WALK_STEP_M ahead of nominal is pulled back toward nominal by the joint
# impedance and the TRUNK is what moves.  Walking is therefore not a new
# swing shape -- it is the same arc with its ENDPOINT moved WALK_STEP_M
# forward, once per swing, which trot_hw feeds through raibert_step_body so
# the reach clamp and the inward cap apply to the walk exactly as to a
# drift correction.
#
# SPEED IS A CONSEQUENCE, NOT A COMMAND: every foot advances WALK_STEP_M
# (ground frame) once per gait cycle, so the trunk travels ~WALK_STEP_M per
# WALL-CLOCK cycle.  For walk_demo that clock includes the settles -- now
# DEMO_SETTLE_S once every DEMO_SETTLE_EVERY cycles, i.e. 0.2/2 = 0.1 s of
# settle per cycle on average: 0.020 m over (1.2 + 0.1) s = ~15 mm/s.
# THE SPEED KNOBS, fastest lever first, and what each one costs:
#     WALK_STEP_M       free until the reach room (below) truncates it --
#                       which is why STAND_TRUNK_BOTTOM_M moved to 0.140
#     DEMO_SETTLE_S / DEMO_SETTLE_EVERY  less settle per cycle walks faster,
#                       paid for in the roll recovery time the settle
#                       exists to provide
#     GAIT_PERIOD       shortens the cycle outright, and re-times the WHOLE
#                       verified gait (stance, swing, handover ramp) -- the
#                       last knob to touch, one run per move
# A quasi-static drag gait tops out around 15-20 mm/s here; genuinely
# faster walking needs the settle gone and a dynamic gait, not a knob.
# The Raibert kv term is left with
# v_cmd = 0 on purpose: in this quasi-static gait the body does not tip onto
# a displaced foot, it is DRAGGED to it, so the dynamic Raibert velocity
# servo has the wrong sign here and the mm-scale kv*v it contributes at
# these speeds is harmless either way.
#
# THE STEP SHARES THE REACH ROOM WITH THE DRIFT CORRECTION.  At the 0.152
# stand height there are only ~10.2 mm of room (see the table by
# MAX_FORWARD_V_AT_STAND_HEIGHT; 0.140 buys 23.6 mm), and both terms are
# clamped TOGETHER -- a WALK_STEP_M near the room means every drift
# correction is truncated.  Keep the sum under the room, or crouch
# (STAND_TRUNK_BOTTOM_M); the runner's banner warns when the step alone
# exceeds it.
#
# READ BY walk_demo.py AND NOTHING ELSE.  trot_hw.main() pins the walk path
# off (args.walk_step = None), so trot_hw and trot_demo trot in place
# whatever this block says -- the verified runners stay exactly what they
# were, and walking is a runner you choose at the command line, not a mode
# a shared switch can leave armed.  WALK_ON's False is walk_demo's own A/B:
# the same runner, same settle clock, feet landing where they lifted.
WALK_ON = True                    # False = walk_demo trots in place too
WALK_STEP_M = 0.020               # m per swing; +x (nose) first, then -x.
                                  # UNDER the ~23.6 mm of reach room at the
                                  # 0.140 height, leaving ~3.6 mm for the
                                  # Raibert correction on top -- about the
                                  # ceiling; past it the corrections clamp.
                                  # At 0.152 the room is 10.2 mm -- shrink
                                  # this to 0.008 before raising the height
# THE RETURN IS THE R KEY, NOT A TIMER (since 2026-08-25; WALK_FWD_S /
# WALK_REV_S are gone).  T walks +x until R is pressed; R walks the SAME
# NUMBER of swings back -- a per-leg ledger (trot_hw.WalkSequence), counted
# in swings and not seconds because the settles make wall time and distance
# different clocks -- then the runner latches the same orderly exit T does:
# HOLD at the next four-foot window, P parks.  There is no forward timeout;
# the operator IS the timer (T latches the exit any time, X stops).

# ===========================================================================
# THE TROT TRACK'S OWN PARAMETER TABLE
# ===========================================================================
# trot_hw.py READS THIS FILE AND NOT torque_stand/params.py.  One folder, one
...
STAND_TRUNK_BOTTOM_M = 0.140            # m  <-- EDIT THIS ONE.  0.140 since
                                        # 2026-08-25: the walk needs the step
                                        # room (23.6 mm vs 10.2 at the 0.152
                                        # nominal); put 0.152 back for
                                        # stand/trot-in-place work
FOOT_RADIUS_M = 0.02
...
TAU_START_MAX = 4.0                      # --tau-max default; raised past the
                                         # 3.0 staged ceiling 2026-08-24 (the
                                         # CLI check now allows up to
                                         # TAU_HARD_NM)
...
def assert_shared(week2_params):
    ...
    `week2_params` is the imported torque_stand.params module.  Called from
    ...
    if bad:
        raise RuntimeError(
            "dog5_trot_quasi_static_model/config.py and torque_stand/params.py disagree on a "
