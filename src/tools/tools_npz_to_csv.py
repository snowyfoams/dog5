...
"""Turn a torque-stand / trot .npz log into something a spreadsheet opens.
    ...
    python3 tools_npz_to_csv.py t1.npz t2.npz t3.npz t5.npz
    python3 tools_npz_to_csv.py *.npz --outdir csv
...
# dog5_paths (imported above) has already put every src/ directory on the
# path.  This used to be a hardcoded absolute path into the robot host's
# home directory, so it only ever worked on that one machine.
