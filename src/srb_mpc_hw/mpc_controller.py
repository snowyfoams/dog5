...
def _swing_frame(C_ib):
    """The rotation the SWING path uses: swing-frame vector = R.T @ body.

    Identity when C.SWING_FRAME_BODY -- the arcs then live in the BODY frame,
    which is trot_hw's verified behaviour: the arc rides the trunk, a rolling
    body does not displace the target relative to the leg, and the foot lands
    under the hip whatever the attitude is doing.  C_ib restores the port's
    original gravity-aligned frame, whose 60 mm-per-10-deg target slingshot
    is what run_20260825_142811 records; see mpc_config.SWING_FRAME_BODY.
    """
    return np.eye(3) if C.SWING_FRAME_BODY else np.asarray(C_ib, dtype=float)
...
class MpcController:
    ...
    def start(self, t, q, state, kine, foot_xy):
        ...
        Rsw = _swing_frame(C_ib)
        self.swing.reset(t, np.array([Rsw.T @ kine.foot_body[i]
                                      for i in range(C.N_LEGS)]))
    ...
    def update(self, t, q, qd, tau_meas, state, kine, plan, z_des,
        ...
        # -- the swing plan -----------------------------------------------
        # Rsw is identity in the body-frame configuration, so feet_rel IS
        # kine.foot_body and the arcs ride the trunk; see _swing_frame.
        Rsw = _swing_frame(C_ib)
        feet_rel = np.array([Rsw.T @ kine.foot_body[i]
                             for i in range(C.N_LEGS)])
        ...
        # ...into the swing frame: v is estimator-world; the body-frame
        # planner wants C v, the gravity-aligned one wants it as-is, and
        # Rsw.T (C v) is both at once.
        v_sw = Rsw.T @ (C_ib @ v_place)
        z_hip = float(state["z_hip"])
        if swing_enabled:
            self.swing.update(t, z_hip, v_sw, self.v_cmd, self.wz_cmd)
        ...
        # layers fight instead of lifting; torque_stand records the run where
...
def effective_gains(mpc=None, stance=None):
    ...
    only form in which it can be compared with the pairs torque_stand walked up
