...
that keeps foot placement out of resonance with itself.
    ...
    TrotGait            NOT redefined here -- dog5_trot_quasi_static_model.gait, imported
...
WHY THE CLOCK IS NOT REDEFINED
    dog5_trot_quasi_static_model/gait.py is already the contact schedule for this robot, in this
...
class ContactAwareGait:
    ...
    dog5_trot_quasi_static_model/trot_hw already does.
    ...
    def update(self, t, measured):
        ...
        if not self.enabled:
            # OFF is the same schedule dog5_trot_quasi_static_model/trot_hw already trots on: the
