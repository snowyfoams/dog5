...
THIS FILE IS dog5_trot_quasi_static_model/trot_hw.py WITH ONE LAYER REPLACED
...
RUN -- supported robot, hand on SPACE
    ...
    sudo HOME=$HOME $E chrt -f 50 python3 srb_mpc_hw/mpc_trot_hw.py --tau-max 1.0

    # the ablation: week 2's wrench in HOLD too, everything else identical.
    sudo HOME=$HOME $E chrt -f 50 python3 srb_mpc_hw/mpc_trot_hw.py \\
        --wrench-hold --log wrench.npz
    sudo HOME=$HOME $E chrt -f 50 python3 srb_mpc_hw/mpc_trot_hw.py --log mpc.npz
    ...
    sudo HOME=$HOME $E chrt -f 50 python3 srb_mpc_hw/mpc_trot_hw.py --w-vel 0
...
# keep in step with that argument, and dog5_trot_quasi_static_model/trot_hw.py is where they
# already live.
...
def run(args):
    ...
    promo = ("ON (--promote)" if args.promote else
             "OFF -- the clock alone, as dog5_trot_quasi_static_model/trot_hw trots today")
    ...
    try:
        ...
        # A FALL DIES ON WHATEVER EXCEPTION THE HARDWARE THROWS -- a CAN
        # write into a tipped-over robot is not a RuntimeError -- and both
        # 2026-08-25 crash runs left NO npz because this handler did not
        # exist, so the save after the try was skipped.  A crashed run is
        # the run whose log matters most; catch, name it, and fall through
        # to the exit report and the save.  Torque is already gone: the
        # MotorBus context unwound before this line.
        import traceback
        traceback.print_exc()
        print(f"\n[mpc] CRASH ({type(exc).__name__}): {exc} -- "
              f"saving the log anyway")
    finally:
...
def main():
    ...
    ap.add_argument("--log", default=None,
                    help="npz path.  EVERY hardware run logs: left unset, a "
                         ...
                         "exactly as trot_hw does.  --no-log is the only "
                         "way to not leave a record.")
    ap.add_argument("--no-log", action="store_true",
                    help="run without writing an npz (bench fiddling only)")
    args = ap.parse_args()

    # THE 2026-08-25 MPC RUNS LEFT NO NPZ AT ALL, because this runner only
    # logged when --log was passed -- so "the MPC looked worse" is a run with
    # no evidence.  trot_hw's convention, verbatim: every run logs unless
    # --no-log says otherwise.
    if args.no_log:
        args.log = None
    ...
    # Up to the DRIVER's saturation, not the staged 3.0: the same relaxation
    # trot_hw made 2026-08-24 when its flown default moved to 4.0, which this
    # runner now inherits (C.TAU_MAX_DEFAULT = tcfg.TAU_START_MAX).
    if not 0.0 < args.tau_max <= C.TAU_HARD_NM:
        ap.error(f"--tau-max must be in (0, {C.TAU_HARD_NM}]")
