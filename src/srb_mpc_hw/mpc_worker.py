...
WHY IT IS A THREAD AND NOT PART OF THE SWEEP -- "cannot", not "should not"
    ...
    torque_primitives/torque_worker.py is the pattern, publishing discipline
...
def mpc_worker(shared, mpc=None, control_hz=C.MPC_HZ, verbose=False):
    ...
    """
    # LEAVE THE RT CLASS BEFORE THE FIRST SOLVE.  The runner is launched
    # under `chrt -f 50`, which makes EVERY thread of the process SCHED_FIFO
    # priority 50 -- this one included -- and equal-priority FIFO threads do
    # not preempt each other, so a ~3 ms solve chunk can sit in front of a
    # CAN slot that had 333 us to leave.  Measured on hardware, 2026-08-25
    # (run_20260825_141317): the sweep stretched to a 5.05 ms median with a
    # 12.5 ms p99 against the drivers' 10 ms input-lost watchdog, and at
    # t = 13.45 s every knee's MEASURED torque went to exactly zero in one
    # tick -- the drivers latched input-lost mid-trot and the robot folded
    # with the commands still ramping into the cap.  SCHED_OTHER for this
    # thread alone puts the sweep strictly above the solver on every core;
    # the solver runs in the gaps, which at ~3 ms a solve it easily can.
    # (What this cannot fix is the GIL: a Python-held lock is inherited, not
    # preempted.  sys.setswitchinterval(0.0005) in the runner bounds each
    # hold; lowering --mpc-hz shrinks how often one happens at all.)
    try:
        os.sched_setscheduler(0, os.SCHED_OTHER, os.sched_param(0))
    except (AttributeError, OSError, PermissionError) as exc:
        print(f"[mpc-worker] could not leave the RT class ({exc}) -- "
              f"sweep timing will suffer under chrt; lower --mpc-hz")
    if mpc is None:
        mpc = ConvexMPC()
